...
type Page<T> = { rows:T[]; hasMore:boolean; owners:{id:string;name:string;username:string}[] };
export function useApiPage<T>(resource: string) {
  const [data,setData] = useState<Page<T>>({rows:[],hasMore:false,owners:[]});
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted) setData(Array.isArray(result) ? {rows:result,hasMore:false,owners:[]} : result);
